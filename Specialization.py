...
class SpecializationApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Медицинская информационная система - Специализации")
    ...
    def setup_ui(self):
        ...
        # Заголовок
        title_label = QLabel("Управление медицинскими специализациями")
        ...
        self.delete_btn.clicked.connect(self.delete_specialization)
        ...
        self.table.setHorizontalHeaderLabels(["ID", "Название специализации"])
    ...
    def load_data(self):
        """Загрузка данных из таблицы specialization"""
        ...
        try:
            self.cursor.execute(
                "SELECT specializationid, specializationname FROM specialization ORDER BY specializationid")
    ...
    def show_add_dialog(self):
        """Диалог добавления новой специализации"""
        dialog = QDialog(self)
        dialog.setWindowTitle("Добавить специализацию")
        ...
        name_input.setPlaceholderText("Введите название специализации")
        ...
        layout.addRow("Название специализации:", name_input)
        layout.addRow(btn_box)

        def add_specialization():
            name = name_input.text().strip()
            if not name:
                QMessageBox.warning(dialog, "Ошибка", "Введите название специализации")
                return

            try:
                self.cursor.execute(
                    "INSERT INTO specialization (specializationname) VALUES (%s) RETURNING specializationid",
                    (name,))
                ...
                self.conn.rollback()
                QMessageBox.critical(dialog, "Ошибка", f"Не удалось добавить специализацию:\n{str(e)}")

        ok_btn.clicked.connect(add_specialization)
        cancel_btn.clicked.connect(dialog.close)

        dialog.exec()

    def show_edit_dialog(self):
        """Диалог редактирования специализации"""
        selected_items = self.table.selectedItems()
        if not selected_items:
            QMessageBox.warning(self, "Ошибка", "Выберите специализацию для редактирования")
            return

        row = selected_items[0].row()
        spec_id = int(self.table.item(row, 0).text())
        current_name = self.table.item(row, 1).text()

        dialog = QDialog(self)
        dialog.setWindowTitle("Редактировать специализацию")
        dialog.setFixedSize(400, 200)
        ...
        def update_specialization():
            ...
            try:
                self.cursor.execute(
                    "UPDATE specialization SET specializationname = %s WHERE specializationid = %s",
                    (new_name, spec_id))
                self.conn.commit()
                ...
                self.conn.rollback()
                QMessageBox.critical(dialog, "Ошибка", f"Не удалось обновить специализацию:\n{str(e)}")

        ok_btn.clicked.connect(update_specialization)
        cancel_btn.clicked.connect(dialog.close)

        dialog.exec()

    def delete_specialization(self):
        """Удаление выбранной специализации"""
        selected_items = self.table.selectedItems()
        if not selected_items:
            QMessageBox.warning(self, "Ошибка", "Выберите специализацию для удаления")
            return

        row = selected_items[0].row()
        spec_id = int(self.table.item(row, 0).text())
        spec_name = self.table.item(row, 1).text()

        reply = QMessageBox.question(
            self, "Подтверждение",
            f"Вы уверены, что хотите удалить специализацию '{spec_name}'?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)

        if reply == QMessageBox.StandardButton.Yes:
            try:
                self.cursor.execute(
                    "DELETE FROM specialization WHERE specializationid = %s",
                    (spec_id,))
                self.conn.commit()
                self.table.removeRow(row)
            except Exception as e:
                self.conn.rollback()
                QMessageBox.critical(self, "Ошибка", f"Не удалось удалить специализацию:\n{str(e)}")
